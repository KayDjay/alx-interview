...
def validUTF8(data):
    ...
    for num in data:
        ...
        if num_bytes == 0:
            ...
            # If the number of bytes is 0, it's a single-byte character
            if num_bytes == 0:
                continue

        else:
            # If the number of bytes is not 0, it means character's position
    ...
    # If there are remaining bytes, it means the data set is incomplete
    return num_bytes == 0
