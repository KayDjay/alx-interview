...
def print_stats(total_size, status_codes):
    """
    Print statistics based on the total file size and the counts of each
    status code.
...
def parse_line(line):
    ...
    Returns:
        tuple or None: A tuple containing the IP address, status code,
        and file size if parsing is successful,
        otherwise None.
    """
    parts = line.split()
    if len(parts) < 9:
...
def main():
    """
    Main function to read log lines from stdin, parse them, compute statistics,
    and print them.
